...
const CMS_ALG_BY_SIG_OID = Object.fromEntries(Object.values(CMS_ALG).map((v) => [v.sigOid, v]));
const CMS_HASH_BY_OID = Object.fromEntries([sha256, sha384, sha512].map((h) => [hashOid(h), h]));
const HASH_NAME_TO_OID = Object.fromEntries(Object.entries({ sha224, sha256, sha384, sha512 }).map(([name, h]) => [name, hashOid(h)]));
...
const ALG_OID_TO_NAME = Object.fromEntries(Object.entries(ALG_NAME_TO_OID).map(([k, v]) => [v, k]));
...
const AlgorithmIdentifier = P.apply(ASN1.sequence({
    algorithm: ASN1.OID,
...
const UTF8String = tagged(0x0c, P.apply(P.bytes(null), {
...
const PrintableString = tagged(0x13, P.validate(ASCII, (s) => {
    if (!/^[A-Za-z0-9 '()+,./:=?-]*$/.test(s))
...
const TeletexString = tagged(0x14, P.apply(P.bytes(null), {
    encode: (b) => {
        let out = '';
...
const NumericString = tagged(0x12, P.validate(ASCII, (s) => {
    if (!/^[0-9 ]*$/.test(s))
        throw new Error(`invalid NumericString: ${JSON.stringify(s)}`);
    return s;
}));
const BMPString = tagged(0x1e, P.apply(P.bytes(null), {
    encode: (b) => {
        if (b.length % 2)
...
const CMSRevocationInfoChoice = ASN1.choice({
...
const CMSSignedData = ASN1.sequence({
    version: ASN1.Integer,
...
const CMS_CONTENT_TYPE_OID_TO_NAME = Object.fromEntries(Object.entries(CMS_CONTENT_TYPE_NAME_TO_OID).map(([k, v]) => [v, k]));
const cmsContentTypeOID = (v) => oidValue(CMS_CONTENT_TYPE_NAME_TO_OID, v, 'CMS contentType');
// RFC 5652 section 3: ContentInfo.
const CMSContentInfo = P.apply(ASN1.sequence({
...
const IPAddress = tagged(0x87, P.apply(P.bytes(null), {
    encode: (b) => {
        if (b.length === 4)
...
const SCTListInner = P.validate(P.apply(P.bytes(null), {
    encode: (b) => 
...
const ExtCRLDP = P.validate(ASN1.sequence({
...
const PolicyQualifierUserNotice = P.apply(ASN1.sequence({
...
const ExtPolicies = P.validate(ASN1.sequence({
    list: P.array(null, ASN1.sequence({
        policy: ASN1.OID,
...
const ExtNameConstraints = P.validate(ASN1.sequence({
...
const ExtSubjectDirectoryAttributes = P.apply(P.validate(ASN1.sequence({
...
const ExtPolicyMappings = ASN1.sequence({
    list: P.array(null, ASN1.sequence({ issuerDomainPolicy: ASN1.OID, subjectDomainPolicy: ASN1.OID })),
});
...
const ExtPolicyConstraints = P.validate(ASN1.sequence({
    ...
    if (x.requireExplicitPolicy === undefined && x.inhibitPolicyMapping === undefined)
        throw new Error('policyConstraints must contain requireExplicitPolicy or inhibitPolicyMapping');
...
const ExtQCStatements = P.apply(ASN1.sequence({
    list: P.array(null, ASN1.sequence({
...
const ExtBasic = P.validate(ASN1.sequence({
...
const EKU_OID_TO_NAME = {
    '2.5.29.37.0': 'anyExtendedKeyUsage',
    '1.3.6.1.5.5.7.3.4': 'emailProtection',
    '1.3.6.1.5.5.7.3.3': 'codeSigning',
};
const EKU_NAME_TO_OID = Object.fromEntries(Object.entries(EKU_OID_TO_NAME).map(([oid, name]) => [name, oid]));
const ExtEKU = P.apply(ASN1.sequence({ list: P.array(null, ASN1.OID) }), {
    ...
    decode: (x) => ({
        list: x.list.map((name) => {
            if (EKU_NAME_TO_OID[name])
                return EKU_NAME_TO_OID[name];
...
const bitFlags = (bs, ix, name) => {
    if (bs.unused > 7)
        throw new Error(`${name} BIT STRING invalid unused bits: ${bs.unused}`);
    const bits = P.array(bs.bytes.length * 8, P.bits(1)).decode(bs.bytes);
    const used = bits.length - bs.unused;
    const get = (i) => (i < used ? !!bits[i] : false);
    const out = {};
    for (const k in ix)
        out[k] = get(ix[k]);
    return out;
...
export const X509 = {
    decode: (der, opts = {}) => X509C.Certificate.decode(berView(der, opts).der),
    encode: (cert) => X509C.Certificate.encode(cert),
    extensions: (cert) => {
        const out = [];
        for (const e of cert.tbs.extensions?.list || []) {
            const body = ExtBody.inner.decode(e.rest);
            const d = { oid: e.oid, critical: !!body.critical };
...
const certInfo = (cert) => {
    ...
    for (const e of exts) {
        ...
        if (e.keyUsage)
            keyUsage = keyUsageBits(e.keyUsage);
        if (e.eku)
            eku = new Set(e.eku.list);
    }
...
const cmsSignedData = (src) => {
    ...
    const signedDataVersion = (() => {
        ...
        if (certs.some((i) => i.TAG === 'other') || crls.some((i) => i.TAG === 'other'))
            return 5n;
        if (certs.some((i) => i.TAG === 'v2AttrCert'))
            return 4n;
        if (certs.some((i) => i.TAG === 'v1AttrCert') ||
    ...
    for (const signerInfo of signedData.signerInfos) {
        const hasDigest = signedData.digestAlgorithms.some((d) => digestAlgEqual(d, signerInfo.digestAlg));
        if (!hasDigest)
...
const cmsCerts = (signedData) => {
    ...
    if (!certs.length)
        throw new Error('SignedData.certificates missing');
    return certs;
};
const cmsSignerInfo = (signedData) => {
    // RFC 5652 section 5.1 allows SET OF SignerInfo; this API profile is single-signer only.
    if (signedData.signerInfos.length !== 1)
        throw new Error(`this API supports exactly one SignerInfo, got ${signedData.signerInfos.length}`);
    ...
    if (!signerInfo)
        throw new Error('SignerInfo[0] missing');
    ...
    if (signerInfo.sid.TAG === 'subjectKeyIdentifier') {
        ...
        if (signerInfo.version !== 3n)
            throw new Error(`SignerInfo.version must be 3 for subjectKeyIdentifier SID, got ${signerInfo.version}`);
    }
    return signerInfo;
};
const cmsVerifyEc = (der, opts = {}) => {
    ...
    const verifyIssuedCert = (child, issuer) => {
        ...
        const alg = CMS_ALG_BY_SIG_OID[sigOid];
        if (!alg)
            throw new Error(`unsupported certificate signatureAlgorithm OID ${sigOid}`);
        if (key.algorithm.info.TAG === 'EC') {
            if (!('ec' in alg))
                throw new Error(`certificate signatureAlgorithm OID ${sigOid} is not compatible with EC issuer key`);
        ...
        if (key.algorithm.info.TAG === 'Ed25519' || key.algorithm.info.TAG === 'Ed448') {
            if (!('ed' in alg) || CMS_ALG[key.algorithm.info.TAG].sigOid !== sigOid)
                throw new Error(`certificate signatureAlgorithm OID ${sigOid} is not compatible with issuer key`);
            if (!CMS_ALG[key.algorithm.info.TAG].ed.verify(child.sig, msg, key.publicKey))
                throw new Error('certificate signature invalid');
            return;
        }
        throw new Error(`unsupported issuer key algorithm ${key.algorithm.info.TAG} for certificate signature verification`);
    };
    const checkCoreCertFields = (cert, where) => {
        ...
        if (v < 0n || v > 2n)
            throw new Error(`${where}: certificate version must be 0..2, got ${v}`);
        ...
        if (cert.tbs.subject.rdns.some((rdn) => rdn.length === 0))
            throw new Error(`${where}: certificate subject distinguished name must not contain empty RDNs`);
        // RFC 5280 sections 4.1.1.2 and 4.1.2.3: Certificate.signatureAlgorithm MUST match TBSCertificate.signature.
        if (!equalBytes(AlgorithmIdentifier.encode(cert.tbs.signature), AlgorithmIdentifier.encode(cert.sigAlg)))
            throw new Error(`${where}: certificate signatureAlgorithm must match tbsCertificate.signature`);
        // RFC 5280 section 4.1.1.3: signatureValue is the certificate signature BIT STRING and must be non-empty.
        if (!cert.sig.length)
            throw new Error(`${where}: certificate signatureValue must be non-empty`);
        ...
        if (proxyExt) {
            ...
            if (!proxyExt.critical)
                throw new Error(`${where}: proxyCertInfo extension must be critical`);
            // RFC 3820 section 3.7: proxy certificate basicConstraints cA MUST NOT be TRUE.
            const isCA = !!exts.find((e) => e.basic)?.basic?.ca;
            if (isCA)
                throw new Error(`${where}: proxy certificate basicConstraints cA MUST NOT be TRUE`);
        }
        // RFC 5280 section 4.2.1.15: freshestCRL extension MUST be non-critical.
        const freshest = exts.find((e) => e.freshestCRL);
        if (freshest?.critical)
            throw new Error(`${where}: freshestCRL extension must be non-critical`);
        ...
        if (nc && !nc.critical)
            throw new Error(`${where}: nameConstraints extension must be critical`);
        ...
        if (nc || pc || iap || pm)
            throw new Error(`${where}: nameConstraints/policyMappings/policyConstraints/inhibitAnyPolicy present but RFC 5280 section 6 processing is not implemented`);
    };
    ...
    if (!signerCert)
        throw new Error('SignerInfo cert not found in certificate set');
    ...
    if (now < timeEpoch(signer.tbs.validity.notBefore) ||
    ...
    if (signerCertInfo.isCA)
        throw new Error('signer certificate must not be a CA certificate');
    ...
    const chainItems = chainSrc.map((c) => {
        if (typeof c === 'string')
            return X509.decode(onePem(c, 'CERTIFICATE').der, opts);
        if (c instanceof Uint8Array)
            return X509.decode(c, opts);
        return X509.decode(X509C.Certificate.encode(c), opts);
    });
    const pool = [
        ...certs.map((c) => X509.decode(X509C.Certificate.encode(c), opts)),
        ...chainItems,
    ].filter((c) => !(equalBytes(subjectDer(c), subjectDer(signer)) && c.tbs.serial === signer.tbs.serial));
    for (const c of pool)
        ensureCritical(c);
    const seen = new Set();
    const chain = [signer];
    ...
    while (true) {
        ...
        if (seen.has(id))
            throw new Error('certificate chain loop detected');
        seen.add(id);
        const curIssuer = X509C.Name.encode(cur.tbs.issuer);
        const curSubject = subjectDer(cur);
        if (equalBytes(curIssuer, curSubject))
            break;
        ...
        const akiState = (issuer) => {
            ...
            if (curAki.authorityCertIssuer) {
                ...
                if (names.length)
                    authorityCertIssuerOk = names.some((n) => equalBytes(X509C.Name.encode(n.data), subjectDer(issuer)));
            }
            const authorityCertSerialOk = curAki.authorityCertSerialNumber === undefined ||
        ...
        if (candidates.length > 1) {
            const akiMatches = curAki
                ...
                : [];
            if (akiMatches.length === 1)
                issuer = akiMatches[0];
            else {
                if (!checkSignatures) {
                    const msg = akiMatches.length > 1
                ...
                if (sigMatches.length === 1)
                    ...
                    throw new Error(`multiple issuer certificates have valid signatures for certificate at depth ${chain.length}`);
                else {
                    if (chain.length === 1)
                        throw new Error('no issuer found for signer in chain');
                    throw new Error(`no issuer found in chain for certificate at depth ${chain.length}`);
        ...
        if (!aki.keyIdentifierOk)
            throw new Error('authorityKeyIdentifier keyIdentifier does not match issuer subjectKeyIdentifier');
        ...
        if (!aki.authorityCertSerialOk)
            throw new Error('authorityKeyIdentifier authorityCertSerialNumber does not match issuer serial');
        // RFC 5280 section 4.2.1.9: certificate-signing issuer must be a CA (basicConstraints cA asserted).
        if (!issuerInfo.isCA)
            throw new Error('issuer certificate is not CA');
    ...
    const checkSignedAttrs = (hash) => {
        ...
        for (const oid in CMS_SIGNED_ATTR_NAME) {
            if (unsigned.some((a) => a.oid === oid))
                throw new Error(`${CMS_SIGNED_ATTR_NAME[oid]} attribute MUST NOT be unsigned`);
        }
        ...
        const getAttrs = (oid) => attrs.filter((a) => a.oid === oid);
        const attrOne = (oid, name) => {
            const all = getAttrs(oid);
            if (all.length !== 1)
                throw new Error(`signedAttrs MUST include exactly one ${name} attribute, got ${all.length}`);
            return all[0];
        };
        const attrZeroOrOne = (oid, name) => {
            const all = getAttrs(oid);
            if (all.length > 1)
                throw new Error(`signedAttrs MUST NOT include multiple ${name} attributes, got ${all.length}`);
            return all[0];
        };
        ...
        if (content === undefined || !hash)
            return;
        const got = ASN1.OctetString.decode(mdAttr.values[0]);
        const exp = hash(content);
        if (!equalBytes(got, exp))
            throw new Error('messageDigest attribute does not match eContent');
    };
    const verifyInputs = (a, verifyOne) => {
        checkDigestAlg(a);
        checkSignedAttrs(a.hash);
        if (signedData.encapContentInfo.eContent === undefined)
            throw new Error('CMS.verify({checkSignatures:true}) requires attached eContent; use CMS.verifyDetached');
    ...
    const sigOid = algOID(signerInfo.signatureAlg.algorithm);
    const sig = CMS_ALG_BY_SIG_OID[sigOid];
    if (!sig)
        throw new Error(`unsupported signatureAlgorithm OID ${sigOid}`);
    if (tag === 'EC') {
        const curve = spkiCurve(key);
        if (!isSignCurve(curve))
        ...
        return verifyInputs(sig, (data) => ecdsaVerifyDer(curve, signerInfo.signature, sig.hash(data), key.publicKey));
    }
    if (tag === 'Ed25519' || tag === 'Ed448') {
        if (!('ed' in sig))
            throw new Error(`unsupported signatureAlgorithm OID ${sigOid}`);
        ...
        return verifyInputs(sig, (data) => CMS_ALG[tag].ed.verify(signerInfo.signature, data, key.publicKey));
    }
...
const cmsSmimeCapabilities = (names) => ASN1.sequence({
    list: P.array(null, ASN1.sequence({
    ...
    list: names.map((name) => {
        ...
        if (n in SMIME_CAPS)
            return { capabilityID: SMIME_CAPS[n] };
        if (/^[0-9]+(?:\.[0-9]+)+$/.test(name))
            return { capabilityID: name };
        throw new Error(`unknown S/MIME capability ${name}`);
    }),
});
const cmsAttrs = (data, algHash, createdTs, smimeCapabilities, messageDigest) => {
    const attrs = [
...
const cmsCertSet = (leaf, chain) => [
    { TAG: 'certificate', data: leaf },
    ...chain.map((c) => ({ TAG: 'certificate', data: c })),
]
    ...
    .map((x) => x.v);
const cmsBuild = (data, leaf, chain, attrs, signature, digestAlgorithm, signatureAlgorithm, digestAlgorithmParams) => {
...
const ecdsaVerifyDer = (curve, signature, msgHash, publicKey) => {
...
const cmsSignCtx = (content, signingCertPem, createdTs, smimeCapabilities, messageDigest, digestAlgorithm) => {
    ...
    const signer = (() => {
        if (leafTag === 'EC') {
            ...
            if (!isSignCurve(curve))
                throw new Error(`cmsSign unsupported signer curve ${curve}`);
            return { tag: 'EC', curve: curve, alg: CMS_ALG[curve] };
        }
    ...
    if (!digestHash)
        throw new Error(`unsupported digestAlgorithm OID ${digestAlgorithm}`);
...
const cmsCompactSign = (signer, leaf, privateKeyPem, toSign, extraEntropy) => {
    ...
    if (keyTag !== signer.tag || kk.kind !== signer.tag)
        throw new Error('cmsSign key type mismatch');
...
export const CMS = {
    decode: (der, opts = {}) => {
        ...
        const contentInfo = CMSX.ContentInfo.decode(ber.der);
        contentInfo.ber = ber;
        return contentInfo;
    },
    encode: (contentInfo) => {
    ...
    contentType: (der, opts = {}) => CMS.decode(der, opts).contentType,
    signed: (der, opts = {}) => {
        return cmsSignedData(berView(der, opts).der).signedData;
    },
    verify: (der, opts = {}) => cmsVerifyEc(der, opts),
    detach: (der, opts = {}) => {
    ...
    verifyDetached: (signature, content, opts = {}) => cmsVerifyEc(CMS.attach(signature, content, opts), opts),
    sign: (content, signingCertPem, privateKeyPem, chainPem = '', opts = {}) => {
        const compactOpts = {
        ...
        return CMS.compact.build(content, CMS.compact.sign(content, signingCertPem, privateKeyPem, {
            ...compactOpts,
            extraEntropy: opts.extraEntropy,
        }), signingCertPem, chainPem, compactBuildOpts);
    },
    signDetached: (content, signingCertPem, privateKeyPem, chainPem = '', opts = {}) => CMS.detach(CMS.sign(content, signingCertPem, privateKeyPem, chainPem, opts)).signature,
    compact: {
        sign: (content, signingCertPem, privateKeyPem, opts = {}) => {
            const c = cmsSignCtx(content, signingCertPem, opts.createdTs, opts.smimeCapabilities, opts.messageDigest, opts.digestAlgorithm);
            return cmsCompactSign(c.signer, c.leaf, privateKeyPem, c.toSign, opts.extraEntropy);
        },
        build: (content, signature, signingCertPem, chainPem = '', opts = {}) => {
            const c = cmsSignCtx(content, signingCertPem, opts.createdTs, opts.smimeCapabilities, opts.messageDigest, opts.digestAlgorithm);
            ...
            if (!signatureAlgorithm)
                throw new Error('signature algorithm OID is required');
            const digestAlgorithmParams = opts.digestAlgorithmParams || 'absent';
            if (c.signer.tag === 'EC')
                ecCurve(c.signer.curve).Signature.fromBytes(signature, 'der');
            else {
                const expected = c.signer.tag === 'Ed25519' ? 64 : 114;
                if (signature.length !== expected)
                    throw new Error(`invalid ${c.signer.tag} signature length: expected ${expected}, got ${signature.length}`);
            }
            const chain = (() => {
                if (!chainPem)
                    return [];
                ...
                if (!blocks.length)
                    throw new Error('no CERTIFICATE PEM blocks found');
                return blocks.map((b) => certItem(b.der));
            })();
            return cmsBuild(c.data, c.leaf, chain, c.attrs, signature, digestAlgorithm, signatureAlgorithm, digestAlgorithmParams);
        },
    },
};
export const __TEST = {
